window.onload = function() {
  ...
  function applyDynamicMenuAccess(userData) {
    ...
    // Блок оплати відображаємо лише для адміністративних ролей.
    ...
    // Рольовий контроль секцій/пунктів меню.
    document.querySelectorAll('[data-roles]').forEach(function(node) {
      const allowedRoles = (node.getAttribute('data-roles') || '')
        ...
        .filter(Boolean);
      if (allowedRoles.length === 0 || allowedRoles.includes(role)) return;
      node.style.display = 'none';
    });

    // Модульний контроль: core завжди доступний.
